...
function searchMeal() {
    ...
    if (!userInput) {
        resultContainer.innerHTML = `<h3 style='color: red;'>Input Field Cannot Be Empty</h3>`;
        return;
    }
    ...
    fetch(apiUrl + userInput).then((response) => response.json()).then((data) => {
        ...
        if (!meal) {
            resultContainer.innerHTML = `<h3 style='color: red;'>No Meal Found, Please Try Again!</h3>`;
            return;
        }
        ...
        .catch(() => {
            ...
            resultContainer.innerHTML = `<h3 style='color: red;'>Couldnt Find the Dish!!</h3>`;
        });
}
